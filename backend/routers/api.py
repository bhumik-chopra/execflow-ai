...
@router.get('/tasks')
async def tasks(request: Request, as_of: datetime = Query(default_factory=datetime.now), classification: Classification | None = None, status: Status | None = None, owner: str | None = None):
...
async def task(task_id: str, request: Request, as_of: datetime = Query(default_factory=datetime.now)):
...
@router.get('/brief')
async def brief(request: Request, as_of: datetime = Query(default_factory=datetime.now)):
    return await request.app.state.engine.brief(as_of)

@router.get('/conflicts')
async def conflicts(request: Request, as_of: datetime = Query(default_factory=datetime.now)):
